# quiz) 단위환산 프로그램
'''
mm 단위의 길이를 입력하면 cm, m, inch, ft 등으로
단위가 변환되어 출력되는 함수가 포함된 프로그램을 만들어봅시다.
'''
...
'''
DW 마트는 고객 감사의 일환으로 ‘오늘의 할인’ 이벤트를 진행할 계획입니다. 
아래의 상품 가격표를 참고해서 ‘오늘의 할인율’을 입력하면
할인된 가격이 출력되는 프로그램을 만들어 봅시다.
쌀: 9,900
상추: 1,900
고추: 2,900
마늘: 8,900
...
def getDiscountPrice(rate):
    ...
    for goodsName in standardPrice.keys():
        disPrice = (standardPrice[goodsName] * (1 - (rate / 100)))
