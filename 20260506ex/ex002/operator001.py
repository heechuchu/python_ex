...
# 컴퓨터 프로그램에서만 사용하는 나머지, 몫, 지수 연산자를 뜻함
# +, -, *, /
# %(나머지), //(몫), ★(지수)★
...
# quiz) dw 전자에서 1분기 수익 계산하려고함
# 사용자가 1분기 매출액과 매입액을 입력하면
# 수익을 계산해주는 프로그램을 만들어보자.
...
# quiz) 가로 세로 길이를 입력하면
# 방의 넓이를 계산해주는 프로그램을 만들어보자
# width = int(input('가로 입력: '))
# length = int(input('세로 입력: '))
# area = width * length
# print(f'방의 넓이: {area}')

# quiz) 'good morning' 문자열을 사용자가 입력한 숫자만큼 출력하는 프로그램
intro = 'good morning'
