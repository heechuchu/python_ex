# 출석부 관리 시스템 만들기
'''
이번에 만들 출석부 관리 시스템은 리스트를 이용해서
 학급의 학생 명단을 관리하는 프로그램으로 시나리오에 따라 
프로그래밍을 전개합니다. 
...
#[[ 시나리오 ]]
 #1 :  학급 학생수가 10명
 # [정우람, 박으뜸, 배힘찬, 천영웅, 신석기, 배민규, 전민수
 # 박건우, 박찬호, 이승엽]인 리스트를 만든다.
 #2 :  ‘가나다’ 순으로 정렬한다.
 #3 :  ‘박찬호’ 학생이 전학을 가게 되었다. 
 # 출석부에서 삭제한 후 전체 학생과 학생 수를 출력한다.
...
# 4 선생님을 돕기 위한 학생으로 앞에서 3명을 뽑는다. (슬라이싱)
...
# 혈액 보관 시스템
'''
헌혈 프로그램을 통해 10명의 기부자에게 혈액을 받아 리스트에 보관하고
보관하고 있는 혈액형을 종류별로 몇백씩 보유 하고 있는지
한눈에 보여주는 프로그램을 만들어봅시다.
'''
LOOP_COUNT = 10
...
print(f'A형: {bloods.count('A')}') # A문자 몇개인지세기
