# 컨테이너 자료형 (list, tuple, dictionary)
# 여러개의 데이터를 묶어서 관리하는것을 말한다.
# list == tuple
...
fruits = ['사과','포도','복숭아']       # 컨테이너자료형: list
print(f'fruits: {fruits}')
print(f'fruits type: {type(fruits)}')


# 파이썬에서 컨테이너 자료형으로는 리스트, 튜플, 딕셔너리

# 리스트(list) 정의(선언 +초기화)
fruits = ['사과','포도','복숭아']

# 인덱스(index): 아이템에 부여된 아이템 식별번호
#    0      1      2  >>인덱스
# ['사과','포도','복숭아'] 
...
print(f'frist data: {fruits[0]}')

# 리스트의 전체 데이터를 조회하는 방법
# 리스트는 반복가능한 객체(데이터)입니다. => 이터러블한 데이터
for fruit in fruits:
    print(f'fruit: {fruit}')  

for idx, fruit in enumerate(fruits):    #  인덱스 , 아이템
    print(f'findex: {idx}, fruit: {fruit}')
...
deletedNum = nums.pop(3) # [1, 2, 3, 5, 6]  deleteditem: 4
...
# remove()를 이용해서 아이템을 삭제할때 삭제하려는
# 아이템의 개수가 2개 이상일때 삭제된다
languages = ['c/c++', 'c#', 'java', 'python']
languages.remove('java')
print(f'languages: {languages}') #['c/c++', 'c#', 'python']

# quiz) 과일 리스트에서 야채를 찾아 삭제하기
'''
다음은 냉장고에 있는 과일을 리스트로 나타낸것입니다.
['사과', '망고', '당근', '수박', '포도', '참외', '토마토']
이중 과일이 아닌 '당근'과 '토마토'를 찾아 삭제하는 프로그램 만들기
'''
...
아래 합격 기준에 만족하는지 구하는 프로그램을 만들어 봅시다.
