...
reverse 옵션이 False면 오름차순(ASC)
True면 내림차순 (DESC)으로 정렬합니다
'''
numbers = [5, 1, 3, 4, 2, 6]
print(f'numbers: {numbers}') # [5, 1, 3, 4, 2, 6]

# 오름차순(ASC)
numbers.sort() # 똑같음 numbers.sort(reverse=False)
...
# 리스트 슬라이싱 (특정아이템 슬라이싱) ★★★★★★★★★★
# 슬라이싱이란, 리스트에서 필요한 부분의 아이템만 뽑아내는것
...
print(f'sliceAnimals: {sliceAnimals}') # ['사자', '곰', '여우']
 ...
 # [n:m] : n 인덱스부터 (m-1) 인덱스까지의 아이템을 슬라이싱(추출)한다.
