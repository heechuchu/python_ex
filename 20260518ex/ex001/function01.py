...
함수를 정의할 때 def 키워드를 사용합니다.
그리고 함수명은 콜론(:), 실행부를 이용합니다.
'''
'''
def 함수명():
    실행부(함수기능)
...
1. 내장함수명과 동일하면 안됨
2. 첫 글자는 주로 소문자로 시작
3. 첫 글자로 숫자를 사용 할수 없다 1greet():안됨 , g1reet(): 됨, greet1(): 됨
4. 특수문자는 사용 할수 없지만 언더바(_)는 사용가능
5. 두개 이상의 단어가 조합되는 경우 스네이크 또는 카멜표기법 사용
...
# quiz) 다국어 인사말 프로그램 함수이용해서 만들기
'''
출신 국가를 선택하면 해당하는 국가의 인사말이
출력되는 프로그램을 함수를 이용해서 만들어봅시다
1. 한국 2. USA 3. Japan
'''
...
연산결과가 출력되는 프로그램을 함수를 이용해 만들어보자
'''
def add():
...
def calculator():
    if selectedOperator == 1:  # 덧셈
        add() #print(f'덧셈 결과: {inputNumber1 + inputNumber2}')
    elif selectedOperator == 2: # 뺄셈
        sub() #print(f'뺄셈 결과: {inputNumber1 - inputNumber2}')
    elif selectedOperator == 3: # 곱셈
        mul()#print(f'뺄셈 결과: {inputNumber1 * inputNumber2}')
    elif selectedOperator == 4: # 나눗셈
        div() #print(f'나눗셈 결과: {inputNumber1 / inputNumber2}')
